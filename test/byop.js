...
Object.create( Object.assign( { }, require('./__proto__'), {
    ...
    runTests() {
        ...
        this.Mocha.describe( `POST /byop`, () => {
            ...
            this.Byop.requiredAttrs.forEach( attr =>
                this.Mocha.it( `Returns 500 when ${attr} not provided`, () => {
                    let byopModel = Object.create( this.Model, { resource: { value: 'byop' }, data: { value: { email: 'topherbaron@gmail.com', total: 123.5 } } } )
                    return byopModel.create()
                    .then( () => {
                        byopModel.data[ attr ] = undefined
                        return this.Request( { path: `/byop`, method: 'POST', body: Object.assign( {}, this.CC.valid, byopModel.data ) } )
                    } )
                    .then( ( [ body, response ] ) => {
                        this.Chai.assert.equal( response.statusCode, 500 )
                        this.Chai.assert.isObject( body )
            ...
            this.Mocha.it( `Returns 500 when total is less than $123.50`, () => {
                let byopModel = Object.create( this.Model, { resource: { value: 'byop' }, data: { value: { email: 'topherbaron@gmail.com', total: 123 } } } )
                return byopModel.create()
                .then( () => this.Request( { path: `/byop`, method: 'POST', body: Object.assign( {}, this.CC.valid, byopModel.data ) } ) )
                .then( ( [ body, response ] ) => {
                    this.Chai.assert.equal( response.statusCode, 500 )
                    this.Chai.assert.isObject( body )
            ...
            this.Mocha.it( `Returns 500 when total is invalid`, () => {
                let byopModel = Object.create( this.Model, { resource: { value: 'byop' }, data: { value: { email: 'topherbaron@gmail.com', total: 150, ace1: true, ace2: true, belmontDonation: 500 } } } )
                return byopModel.create()
                .then( () => this.Request( { path: `/byop`, method: 'POST', body: Object.assign( {}, this.CC.valid, byopModel.data ) } ) )
                .then( ( [ body, response ] ) => {
                    this.Chai.assert.equal( response.statusCode, 500 )
            ...
            this.Mocha.it( `Returns 200 creates byop row, stripe charge when shit is all there`, () => {
                let byopModel = Object.create( this.Model, { resource: { value: 'byop' }, data: { value: { email: 'topherbaron@gmail.com', total: 124.50, ace1: false, ace2: false, belmontDonation: 1 } } } )
                return byopModel.create()
                .then( () => this.Request( { path: `/byop`, method: 'POST', body: Object.assign( {}, this.CC.valid, byopModel.data ) } ) )
                .then( ( [ body, response ] ) => {
                    ...
                    .then( rows => {
                        ...
                        this.Byop.requiredAttrs.forEach( attr => this.Chai.assert.equal( row[ attr ], byopModel.data[ attr ] ) )
            ...
            this.Mocha.it( `Returns 200, 'All slots have filled.  You have been added to the wait list.' when shit is all there, but there are already 108 teams signed up for saturday`, () => {
                ...
                let byopModel = Object.create(
                    this.Model, {
                        resource: { value: 'byop' },
                        data: { value: { email: 'topherbaron@gmail.com', total: 124.50, ace1: false, ace2: false, belmontDonation: 1, divisionId: recId, waitList: false, stripeChargeId: null, hasPaid: false } }
                    }
                )
                return byopModel.create()
                .then( () => Promise.all( this.getIntRange( 108 ).map( () => this.Postgres.insert( 'byop', byopModel.data ) ) ) )
                .then( () => this.Request( { path: `/byop`, method: 'POST', body: Object.assign( {}, this.CC.valid, byopModel.data ) } ) )
                .then( ( [ body, response ] ) => {
                    this.Chai.assert.equal( response.statusCode, 200 )
                    this.Chai.assert.isObject( body )
            ...
            this.Mocha.it( `Returns 200, 'You have been added to the wait list.' when shit is all there except CC info, and there are already 108 teams signed up for sunday`, () => {
                ...
                return byopModel.create()
                .then( () => Promise.all( this.getIntRange( 108 ).map( () => this.Postgres.insert( 'byop', byopModel.data ) ) ) )
                .then( () => this.Request( { path: `/byop`, method: 'POST', body: Object.assign( {}, byopModel.data ) } ) )
                .then( ( [ body, response ] ) => {
                    this.Chai.assert.equal( response.statusCode, 200 )
                    this.Chai.assert.isObject( body )
            ...
            this.Mocha.it( `Returns 500, 'Error processing payment.  Please try again.' when shit is all there except CC info is invalid`, () => {
                let byopModel = Object.create(
                    this.Model, {
                        resource: { value: 'byop' },
                        data: { value: { email: 'topherbaron@gmail.com', total: 124.50, ace1: false, ace2: false, belmontDonation: 1, waitList: false, stripeChargeId: null, hasPaid: false } }
                    }
                )
                return byopModel.create()
                .then( () => this.Request( { path: `/byop`, method: 'POST', body: Object.assign( {}, this.CC.invalid, byopModel.data ) } ) )
                .then( ( [ body, response ] ) => {
                    this.Chai.assert.equal( response.statusCode, 500 )
                    this.Chai.assert.isObject( body )
        ...
        this.Mocha.describe( `DELETE /byop`, () => {
            this.Mocha.it( `Returns 404`, () =>
                this.Request( { path: `/byop`, method: 'DELETE', body: { } } )
                .then( ( [ body, response ] ) => {
                    this.Chai.assert.equal( response.statusCode, 404 )
                    this.Chai.assert.isObject( body )
                    this.Chai.assert.equal( Object.keys( body ).length, 0 )
